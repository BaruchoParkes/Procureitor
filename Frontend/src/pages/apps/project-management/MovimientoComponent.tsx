...
import {
  TipoDeMovimiento,
  tipoDeMtoInicial
} from 'data/project-management/tipoDeMovimiento';
...
const MovimientoComponent = () => {
  ...
  const autos_cortos = `${mto.Proc.ACTO.split(' ')[0]} C/ ${
    mto.Proc.DEMA.split(' ')[0]
  } S/ ${mto.Proc.TPRO.split(' ')[0]}`;
  ...
  useEffect(() => {
    const fetchMto = async () => {
      try {
        const response = await axios.get(`${apiUrl}/mtos/id/${id}`);
  ...
  useEffect(() => {
    const fetchTMto = async () => {
      try {
        const response = await axios.get(`${apiUrl}/tMtos`);
  ...
  useEffect(() => {
    ...
    if (mto.tipoDeMovimiento && tipos.length > 0) {
      try {
        const selectedTipo =
          tipos.find(tipo => tipo.tipoMtoID === mto.tipoDeMovimiento) ||
          tipoDeMtoInicial;
  ...
  useEffect(() => {
    const fetchCobro = async () => {
      if (mto.cobros_fk && mto.cobros_fk !== 0) {
        try {
          const response = await axios.get(`${apiUrl}/cobros/id/${mto.cobros_fk}`);
          console.log('Fetched cobro', response.data);
          setCobro(response.data);
        } catch (error) {
          console.error(
            'Error fetching cobro:',
  ...
  useEffect(() => {
    ...
    if (isSaleTranfeActive && (cobro.monto || cobro.quien_cobra)) {
      const newDescripcion = `Cobro: $${cobro.monto}(${
        cobro.capital_honorarios
      }) ${
        cobro.capital_honorarios == 'Capital' ? ' - PCL: $' + cobro.PCL : ''
      }- Cobra: ${cobro.quien_cobra || 'Efectivo'} - ${cobro.estado}`;
      console.log('Setting cobro descripcion:', newDescripcion);
      const newNombreCobro = `${autos_cortos || ''} - ${
        cobro.capital_honorarios
      } - $ ${cobro.monto} - PCL: $${cobro.PCL}-  Cobra: ${
        cobro.quien_cobra || 'Efectivo'
      }`;
  ...
  const handleTMtoSelectChange = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
    const selectedValue = event.target.value;
    const selectedTipo =
      tipos.find(tipo => tipo.movimiento === selectedValue) || tipoDeMtoInicial;
    setTmto(selectedTipo);
    setTipoDelTipo(selectedTipo.tipo);
    setMto(prev => ({
      ...prev,
  ...
  const handleMontoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch (tmto.tipoMtoID) {
      case '01':
        ...
        concepto = 'Capital';
        estado = 'Pendiente';
        if (mto?.Proc?.Ojud?.nombre_corto?.substring(0, 3) === 'SRT') {
  ...
  const handleCHchange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const newNombreCobro = `${autos_cortos || ''} - ${
      cobro.capital_honorarios
    } - $ ${cobro.monto} - Cobra: ${cobro.quien_cobra || 'Efectivo'}`;
  ...
  const handleQuienCobraChange = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
    setCobro(prev => ({ ...prev, quien_cobra: event.target.value }));
  };

  const handleSelectChangeTranfeOLJ = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
    setCobro(prev => ({
      ...prev,
      libranza_judicial_transferencia_directa: event.target.value
    }));
  };
  ...
  const handleSelectChangeCobrado = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
  ...
  const handleSubmit = async (
    ...
    updatedMto?: Movimiento
  ) => {
    e.preventDefault();
    try {
      const mtoToSave = updatedMto || mto;
      await axios.post(`${apiUrl}/mtos/update`, {
        id: mtoToSave.mtoId,
  ...
  const handleSubmitCobro = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const response = await axios.post(`${apiUrl}/cobros/store`, cobroData);
          ...
          <Row className="g-3">
            ...
            <Col xs={12} md={4}>
              ...
              >
                ...
                <option value="Transferencia Directa">
