...
const ProcesoDetails = () => {
  ...
  useEffect(() => {
    const fetchProc = async () => {
      try {
        const response = await axios.get(`${apiUrl}/procesos/id/${id}`);
  ...
  useEffect(() => {
    const fetchMtos = async () => {
      try {
        const response = await axios.get(`${apiUrl}/mtos/proc/${id}`);
        const data = await response.data;
        setMtos(data);
      } catch (error) {
        console.error('ha habido un error: ', error);
  ...
  useEffect(() => {
    const fetchEstados = async () => {
      try {
        const response = await axios.get(`${apiUrl}/procEstM2m/proc/${id}`);
        const data = await response.data;
        setestados(data);
      } catch (error) {
        console.error('ha habido un error: ', error);
  ...
  useEffect(() => {
    const fetchTEstados = async () => {
      try {
        const response = await axios.get(`${apiUrl}/tEstado`);
        const data = await response.data;
  ...
  const handleTEstadoSelectChange = async (
    selectedOption: SingleValue<EstadoOption>
  ) => {
    if (!selectedOption) return; // Handle the case when selection is cleared
    const selectedEstado = selectedOption.value;
    try {
      await axios.post(`${apiUrl}/procEstM2m/create`, {
        proc: id,
        estado: selectedEstado
      });
      console.log(
        `Successfully updated proc ${id} with estado ${selectedEstado}`
      );
    } catch (error) {
  ...
  const handleArchivarButton = () => {
    axios
      .post(`${apiUrl}/procesos/archivar`, { PROC: proc.PROC })
      .then(response => {
  ...
  const handleButtonClickNewMto = async (
    e: React.MouseEvent<HTMLButtonElement>
  ) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${apiUrl}/mtos/store`, {
        ...
        <div className="px-4 px-lg-6 pt-6 pb-9">
          ...
          <Row className="gx-0 gx-sm-5 gy-8 mb-8">
            ...
            startIcon={<FontAwesomeIcon icon={faPlus} className="fs-10 me-2" />} */
                ...
                <Button variant="primary" onClick={handleArchivarButton}>
                  Archivar
                </Button>
              </div>
            </Col>
