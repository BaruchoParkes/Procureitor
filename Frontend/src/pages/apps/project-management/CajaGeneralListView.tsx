...
const CajaGeneralListView = () => {
  ...
  useEffect(() => {
    const fetchCaja = async () => {
      try {
        const response = await axios.get(`${apiUrl}/caja/${user?.iniciales}`);
    ...
    <div>
      <AdvanceTableProvider {...table}>
        <div className="d-flex flex-wrap mb-4 gap-3 gap-sm-6 align-items-center">
          <h2 className="mb-0">
            <span className="me-3">Caja General</span>
          </h2>
        </div>
        <CajaListTable />
...
export default CajaGeneralListView;
