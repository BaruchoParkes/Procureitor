...
import CobrosListTable, {
  cobrosListTableColumns
} from 'components/tables/CobrosListTable';
    ...
    const fetchCobros = async () => {
      try {
        const response = await axios.get(
          `${apiUrl}/cobros/pendientes/usuario/${user?.iniciales}`
        );
      ...
      <AdvanceTableProvider {...table}>
        <div className="d-flex flex-wrap mb-4 gap-3 gap-sm-6 align-items-center">
          <h2 className="mb-0">
            ...
            <div className="fw-normal text-body-tertiary"></div>
          </h2>
        </div>
        <div>
          <h4 className="mb-0">
            <div className="me-5">
              Cargados por mí o que yo tengo que cobrar
            </div>{' '}
            <div className="fw-normal text-body-tertiary"></div>
          </h4>
        </div>
